...
Or::Or(sf::Vector2i mouse_position) {
	_start_color = sf::Color::Green;
	...
	nodes.in2.setPosition(_sprite.getPosition().x - _sprite.getSize().x / 2., _sprite.getPosition().y + 60);
	nodes.out1.setPosition(_sprite.getPosition().x + _sprite.getSize().x / 2., _sprite.getPosition().y - 4);
...
void Or::Draw(sf::RenderTarget& window) {
	window.draw(_sprite);
